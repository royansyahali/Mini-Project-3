...
def fun_top_country_get_data(**kwargs):
    spark = (
...
def fun_top_country_load_data(**kwargs):
    ...
    df.to_sql(name="top_country_royan", con=engine)
...
def fun_total_film_load_data(**kwargs):
    ...
    df.to_sql(name="total_film_royan", con=engine)
...
with DAG(
    ...
    op_top_countries_get_data = PythonOperator(
        task_id="top_country_get_data", python_callable=fun_top_country_get_data
    )

    op_top_countries_load_data = PythonOperator(
        task_id="top_country_load_data", python_callable=fun_top_country_load_data
    )

    # Task 2
    op_total_film_get_data = PythonOperator(
        task_id="total_film_get_data", python_callable=fun_total_film_get_data
    )

    op_total_film_load_data = PythonOperator(
        task_id="total_film_load_data", python_callable=fun_total_film_load_data
    )
